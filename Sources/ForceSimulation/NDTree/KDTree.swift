...
/// The data structure carried by a node of GenericTree
...
extension KDTree {
    ...
    public final class GenericTree<V, D> where V: VectorLike, D: KDTreeDelegate, D.V == V {
        ...
        public private(set) var children: [GenericTree<V, D>]?
        ...
        private func addWithoutCover(_ nodeIndex: NodeIndex, at point: V) {
            ...
            guard let children = self.children else {
                    ...
                    ) < clusterDistanceSquared
                {
                    ...
                    let spawned = GenericTree<V, D>.spawnChildren(
                        box,
        ...
        private func expand(towards direction: Direction) {
            let nailedDirection = (GenericTree<V, D>.directionCount - 1) - direction
        ...
        /// The children count of a node in GenericTree.
        ...
        private static func spawnChildren(
            ...
            var result = [GenericTree<V, D>]()
            result.reserveCapacity(GenericTree<V, D>.directionCount)
            let center = _box.center

            for j in 0..<GenericTree<V, D>.directionCount {
                var __box = _box
                ...
                result.append(
                    GenericTree<V, D>(
                        box: __box, clusterDistance: _clusterDistance,
                        parentDelegate: /*&*/ _delegate)
                )
            }
        ...
        private func shallowCopy() -> GenericTree<V, D> {
            let copy = GenericTree(
        ...
        @inlinable public func visit(shouldVisitChildren: (GenericTree<V, D>) -> Bool) {
